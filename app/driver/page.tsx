    ...
    return (
        <div className="space-y-6 pb-20 lg:pb-0">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                ...
                <Button className="hidden lg:flex bg-green-600 hover:bg-green-700 shadow-md hover:shadow-lg transition-all">
            ...
            <div className="grid gap-4 md:gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
                <KPICard
                    title="Current Trip"
                    value={driverData?.currentTrip?.vehicle || 'No Active Trip'}
                    icon={MapPin}
                    accentColor="blue"
                    subtitle={driverData?.currentTrip?.route || 'Waiting for assignment'}
                />
                <KPICard
                    title="Hours Today"
                    value={`${driverData?.stats?.hoursToday || 0}h`}
                    icon={Clock}
                    accentColor="orange"
                    subtitle="Out of 8h shift"
                />
                <KPICard
                    title="This Month"
                    value={driverData?.stats?.tripsThisMonth || 0}
                    icon={CheckCircle}
                    accentColor="purple"
                ...
                <KPICard
                    title="Earnings"
                    value={`₹${driverData?.stats?.earningsThisMonth?.toLocaleString('en-IN') || 0}`}
                    icon={DollarSign}
                    accentColor="green"
                    trend={{ value: "This month", positive: true, label: "" }}
                />
            </div>
                        ...
                        {driverData?.currentTrip ? (
                            <div className="space-y-4">
                                ...
                                <div className="pt-4 space-y-2">
                                    <Button className="w-full bg-blue-600 hover:bg-blue-700 h-12 text-lg">Update Location</Button>
                                    <Button variant="outline" className="w-full h-12">Mark Completed</Button>
                                </div>
                            </div>
                        ) : (
            ...
            {/* Sticky Mobile Start Trip Button */}
            <div className="fixed bottom-0 left-0 right-0 p-4 bg-white dark:bg-gray-900 border-t lg:hidden z-50">
